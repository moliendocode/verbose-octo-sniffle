const formatValues = (data: any) => {
    ...
    data.dolar_intercambio.fecha = new Date(
        data.dolar_intercambio.fecha
    ...
    data.libra_cobre.fecha = new Date(
        data.libra_cobre.fecha
    ).toLocaleDateString("es-CL");
    data.tasa_desempleo.fecha = new Date(
        data.tasa_desempleo.fecha
    ).toLocaleDateString("es-CL");
    data.bitcoin.fecha = new Date(data.bitcoin.fecha).toLocaleDateString(
        "es-CL"
    );
    ...
    data.dolar_intercambio.valor =
        ...
        new Intl.NumberFormat().format(
            Math.round(data.dolar_intercambio.valor)
        );
    data.euro.valor =
